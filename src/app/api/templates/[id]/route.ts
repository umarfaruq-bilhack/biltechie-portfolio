...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const updates = { ...body }
  if (body.stack) updates.stack = toArr(body.stack)
  if (body.features) updates.features = toArr(body.features)
  const { data, error } = await supabaseAdmin().from('templates').update(updates).eq('id', params.id).select().single()
