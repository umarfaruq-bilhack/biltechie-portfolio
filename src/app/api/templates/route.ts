...
export async function GET() {
  ...
  const query = admin
    ? supabaseAdmin().from('templates').select('*').order('sort_order')
    : supabase.from('templates').select('*').eq('visible', true).order('sort_order')
...
export async function POST(req: Request) {
  ...
  const { data, error } = await supabaseAdmin().from('templates').insert({
    ...body, stack: toArr(body.stack), features: toArr(body.features)
  }).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json(data)
}
